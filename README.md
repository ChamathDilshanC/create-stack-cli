...
- **Rust backend (Axum)** — a hand-written `Cargo.toml` + `src/main.rs` with a working `GET /` route; Cargo itself resolves and builds dependencies on the first `cargo run`, so there's no separate install step.
...
| **Rust toolchain (Cargo)** | only if you pick the Axum (Rust) backend, and only to actually build/run the generated project afterward — scaffolding itself just writes `Cargo.toml` + `src/main.rs` by hand, no local Rust needed |
...
| Language | unless the framework forces one | TypeScript (default), JavaScript — or nothing at all for Python/Java/Rust frameworks |
...
| Code quality | always | **Node:** ESLint + Prettier, Biome, None · **Python:** Ruff, Black + Flake8, None — not yet automated for Spring Boot or Rust (`cargo fmt`/`cargo clippy` already ship with the toolchain) |
...
| Package manager | Node frameworks only | npm, yarn, pnpm, bun — Python always uses pip in a fresh `.venv`, Spring Boot always uses its own Maven/Gradle wrapper, Rust always uses Cargo |
| Install now | always | Yes / No — Spring Boot and Rust skip this too, since their wrapper/Cargo resolves dependencies itself on first run |

Angular and NestJS force TypeScript; Django, Flask, FastAPI, and the AI/ML project type force Python; Spring Boot forces Java; Axum forces Rust — each also skips the package-manager question entirely. Trying to answer any of those with a flag just gets silently overridden to the correct value, the same way it would if you answered the prompt "wrong."
...
| **Backend** | Express.js, NestJS, Fastify, Hono, Django, Flask, FastAPI, Spring Boot, Axum (Rust) | the Nest CLI (`nest new`) · `create-hono` · `django-admin startproject` · start.spring.io's `starter.zip` · hand-written for Express/Fastify/Flask/FastAPI/Axum (none has an official scaffolder) |
...
| `-l, --language <lang>` | `ts` or `js` — ignored (and unnecessary) for Python/Java/Rust frameworks |
